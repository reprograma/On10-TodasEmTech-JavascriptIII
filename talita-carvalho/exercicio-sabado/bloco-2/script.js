...
const frutas = ['banana', 'maçã', 'abacaxi', 'abacate', 'cajú']


//2. Crie uma array com 100 espaços vazios.
const numerosVazios = Array(100)
...
//  ESTUDAR MAIS SOBRE O FOR E SUA ESTRUTURA
...
for(let index = 0; index < numerosVazios.length; index++ ){
  numerosVazios[index] = index + 1
}
...
const numeroVazio = Array(100);

for (let index = 0; index < numeroVazio.length; index++) {
  numeroVazio[index] = index + 1;
}

numeroVazio.forEach(function (item) {
  console.log(item * 2);
});

...
```
1. O que é iteração?
    São repetições de um trecho de código.

2. Qual a diferença de uma function padrão e a arrow function?
    A forma em que é escrita. Arrow Function é uma forma menor de escrita e mais utilizada por melhor qualidade de código.

3. Como funciona o método map()? 
    É uma função que chama outra função como argumento. Como o próprio nome diz, ela mapeia um array para executar a função callback nela inserida como argumento.
```

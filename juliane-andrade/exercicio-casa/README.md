...
#### RESPOSTA=
Iteração é o processo de repetição de determinada ação.

2. Qual a diferença de uma function padrão e a arrow function?
#### RESPOSTA=
A arrow function ela possui uma sintaxe menor que a função padrão, ela não é nomeada, não possui seu próprio diz, arguments, super ou new.target. Diferentemente da function padrão que possui.

3. Como funciona o método map()? 
#### RESPOSTA=
O método Map() percorre todo array, chamando a função callback para casa elemento do array, e retorno um novo array.
```

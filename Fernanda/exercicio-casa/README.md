...
```
1. O que é iteração?
Resposta : É um processo de repetição.

2. Qual a diferença de uma function padrão e a arrow function?
Resposta: A diferença é que a function padrão precisa declaração e tem um retorno, já a arrow function é um tipo de função mais curta. ela não precisa necessariamente de declaração e tbm omitida as chaves não precisa do return.

3. Como funciona o método map()?
resposta: O método map() funciona a partir da invocação de um array e tem como parametro uma função de callback, que por sua vez, é invocada para cada item e tem o retorno o array resultante. 
```

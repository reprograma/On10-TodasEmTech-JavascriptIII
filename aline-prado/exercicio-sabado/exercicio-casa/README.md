...
```
1. O que é iteração?
R:É o processo de repetição, realizados por alguns métodos ou arrays.

2. Qual a diferença de uma function padrão e a arrow function?
R: A arrow function tem uma estrutura mais simples que a function padrão. A arrow function também não precisa de uma nomenclatura ou chaves, numa função simples.

3. Como funciona o método map()? 
R: O mpétodo map, chama a função callback, que passando por cada elemento do array, gerando um novo array.
